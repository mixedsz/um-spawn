...
local function ToggleSync(status)
    if status then
        if um.weather.sc == 'vSync' then
            TriggerEvent('vSync:toggle', true)
            TriggerEvent('vSync:updateWeather', um.weather.type, false)
...
function GetProperties()
    local function getPropertyStatusAndData(status, callbackName)
            ...
            if callbackName == 'getHouses' then
                ...
                for i = 1, (#result), 1 do
                    ...
                    processedHouses[#processedHouses + 1] = {
                        house = houseData.Entrance or houseData.house,
                        label = houseData.Name or houseData.house
                    }
                end
